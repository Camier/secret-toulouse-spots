...
def generate_report():
    ...
    for source, total, with_coords, hidden in sources:
        ...
        print(f"  {source}: {total} spots ({coord_pct:.0f}% with coords, {hidden_pct:.0f}% hidden)")
    ...
    if stats[2] > stats[0] * 0.3:  # More than 30% without coords
        recommendations.append(f"High priority: {stats[2]} spots ({stats[2]/stats[0]*100:.0f}%) missing coordinates - run geocoding")
    
    if short_names > 5:
        recommendations.append(f"Fix {short_names} spots with short names")
    
    if duplicate_groups > 5:
        recommendations.append(f"Review and merge {duplicate_groups} potential duplicate groups")
    
    if no_activities > stats[0] * 0.2:  # More than 20% without activities
        recommendations.append(f"Enrich {no_activities} spots with proper activity tags")
