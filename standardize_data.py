...
class SpotStandardizer:
    ...
    def standardize_all(self):
        ...
        if duplicates:
            ...
            for main_id, dups in list(dup_groups.items())[:5]:
                self.cursor.execute(
                    "SELECT extracted_name FROM spots WHERE id = ?", (main_id,)
                )
                main_name = self.cursor.fetchone()[0]
                print(f"\n  '{main_name}' (ID: {main_id})")
                for dup_id, reason in dups:
                    self.cursor.execute(
                        "SELECT extracted_name FROM spots WHERE id = ?", (dup_id,)
                    )
    ...
    def generate_report(self):
        ...
        self.cursor.execute(
            "SELECT location_type, COUNT(*) FROM spots GROUP BY location_type"
        )
