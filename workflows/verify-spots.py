...
def verify_spot_data(conn):
    ...
    cursor.execute(
        "SELECT COUNT(*) FROM spots WHERE extracted_name IS NULL OR extracted_name = ''"
    )
    ...
    cursor.execute(
        "SELECT id, extracted_name, latitude, longitude FROM spots WHERE latitude IS NOT NULL"
    )
    out_of_range = []
    ...
    for spot in cursor.fetchall():
        ...
        if not valid:
            out_of_range.append({"id": spot[0], "name": spot[1], "reason": msg})
...
def main():
    ...
    if duplicates:
        ...
        for dup in duplicates[:5]:  # Show first 5
            print(
                f"   - {dup['spot1']['name']} ↔️ {dup['spot2']['name']} ({dup['distance_m']}m apart)"
            )
    else:
    ...
    stats = (
        f"{len(duplicates)} duplicates, {issues['missing_coordinates']} missing coords"
    )
    subprocess.run(
        [
