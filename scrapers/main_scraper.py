...
# Import our scrapers
from test_camptocamp_real import scrape_camptocamp_outings

try:
    from instagram_scraper_secure import (
        SecureInstagramScraper as FrenchOutdoorInstagramScraper,
    )
except ImportError:
    ...
    def export_for_weather_app(self):
        ...
        for name, lat, lng, source, text, is_hidden in locations:
            ...
            if any(word in (text or "").lower() for word in ["cascade", "chute"]):
                ...
                for word in ["baignade", "piscine naturelle"]
            ):
...
def main():
    ...
    parser.add_argument(
        "--camptocamp", action="store_true", help="Run only CampToCamp scraper"
    )
    parser.add_argument(
        "--instagram", action="store_true", help="Run only Instagram scraper"
    )
    parser.add_argument(
        "--export", action="store_true", help="Export data for weather app"
    )
