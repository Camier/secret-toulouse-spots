...
class FrenchLocationExtractor:
    ...
    def _setup_location_patterns(self):
        ...
        patterns = [
            ...
            [
                {
                    "LOWER": {
                        "IN": ["col", "pic", "mont", "sommet", "crête", "aiguille"]
                    }
                },
    ...
    def _is_hidden_spot(
        self, full_text: str, start: int, end: int, window: int = 200
    ) -> bool:
        ...
        return {
            **self.stats,
            "unique_locations": len(
                set(loc["text"] for loc in self.extracted_locations)
            ),
            "hidden_spots": sum(
                1 for loc in self.extracted_locations if loc.get("is_hidden", False)
            ),
        }
...
def main():
    ...
    for text in test_texts:
        ...
        for loc in locations:
            print(
                f"  - {loc['text']} ({loc['type']}, {loc.get('geo_classification', 'unknown')})"
            )
